...
mod native {
    ...
    // Uses RwLock for thread-safe combined state
    // ──────────────────────────────────────────────────────────────
    ...
    fn add_filter_id(id: u64) {
        let mut state = FILTER_STATE.write().unwrap();
        state.ids.push(id);
    ...
    fn has_filter_ids() -> bool {
        FILTER_STATE.read().unwrap().installed
    }
    ...
    impl WfpEngine {
        ...
        pub fn install_kill_switch(&mut self, config: &KillSwitchConfig) -> Result<()> {
            ...
            match result {
                Ok(()) => {
                    ...
                    WFP_FILTERS_INSTALLED.store(true, Ordering::SeqCst);
                    let count = FILTER_STATE.read().unwrap().ids.len();
        ...
        /// CRITICAL FIX: Restores filter IDs on failure to allow retry.
        pub fn remove_filters(&mut self) -> Result<()> {
            ...
            // Take all filter IDs atomically
            let filter_ids = take_filter_ids();
            ...
            for id in &filter_ids {
                ...
                } else if status == 0x80070002 {
                    // ERROR_FILE_NOT_FOUND — already removed
                    success_count += 1;
                    info!("wfp filter {id} already removed");
                } else {
                    warn!("FwpmFilterDeleteById0({id}) failed: 0x{status:08x}");
                    errors.push(format!("filter_id_{id}: 0x{status:08x}"));
                }
            }

            // CRITICAL: Only mark clean if ALL filters removed successfully
            // If any failed, restore IDs so retry can attempt again
            if errors.is_empty() {
                WFP_FILTERS_INSTALLED.store(false, Ordering::SeqCst);
                info!("wfp: all {} filters removed", success_count);
                Ok(())
            } else {
                // FAILED - restore filter IDs and state for retry
                let mut ids = FILTER_STATE.write().unwrap();
                ids.ids
                    .extend(filter_ids.into_iter().skip(success_count as usize));
                ids.installed = true;
                WFP_FILTERS_INSTALLED.store(true, Ordering::SeqCst);
                self.cleaned.store(false, Ordering::SeqCst);
                Err(anyhow!(
                    "wfp filter removal failed: {} errors, {} filters may persist",
                    errors.len(),
                    ids.ids.len()
                ))
            }
    ...
    // ──────────────────────────────────────────────────────────────
    // Public API — filter recovery and installation
    // ──────────────────────────────────────────────────────────────
                ...
                if !name_ptr.is_null() {
                    ...
                    if name.contains("Aegis") || name.contains("aegis") {
                        ...
                        if del_status == 0 {
                            ...
                            warn!(
                                "failed to delete orphaned filter {}: 0x{:08x}",
                                filter_id, del_status
                            );
                        }
                    }
                }
    ...
    /// Remove all filters (crash-safe, idempotent).
    pub fn remove_filters() -> Result<()> {
        if !WFP_FILTERS_INSTALLED.load(Ordering::SeqCst) && !has_filter_ids() {
            info!("wfp: no filters to remove (cached state)");
            return Ok(());
        }

        match WfpEngine::open() {
            Ok(mut engine) => engine.remove_filters(),
            Err(e) => {
                warn!("wfp engine not available for cleanup: {e}");
                Ok(())
            }
        }
    }

    /// Install kill switch filters (admin-aware with firewall fallback).
    /// This is called from the higher-level NativeWfpController.
