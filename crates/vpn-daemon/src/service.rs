    ...
    match req {
        ...
        IpcRequest::Metrics => IpcResponse::Metrics {
            metrics: vpn_ipc::DaemonMetrics {
                ...
                error_rate: {
                    let req = runtime.request_count.load(Ordering::Relaxed);
                    let err = runtime.error_count.load(Ordering::Relaxed);
                    if req == 0 { 0.0 } else { err as f64 / req as f64 }
                },
                connect_count: runtime.connect_count.load(Ordering::Relaxed),
                disconnect_count: runtime.disconnect_count.load(Ordering::Relaxed),
        ...
        IpcRequest::Disconnect { admin_secret } => {
            if !authorize_disconnect(runtime.admin_secret.as_deref(), admin_secret.as_deref()) {
                ...
                runtime.shutdown.store(true, Ordering::Relaxed);
                runtime.disconnect_count.fetch_add(1, Ordering::Relaxed);
